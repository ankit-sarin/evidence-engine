...
| 4 | Order of sessions and steps, with gates | Active: four named states; sessions 1–5 closed; session 6 (READERS-01) Part 0 and Phases 1, 2a and 2b closed; Phase 3 pending |
...
| 6 | Readers onto the reader | S2 readers · S1c · S3h | Concordance, exporter, judge loader and corpus predicate read through the reader; the D1-4 reproducer passes; a judge run under S1c attempts 3,802 cells; `EXTRACT_FAILED` papers stay eligible with a reason — **Revised 2026-09-21 (R28, R29):** the *"attempts 3,802 cells"* clause is **moot** under R25, which removed the legacy field-level history that number counted, and is replaced by R28: the judge loader takes the **full cell grid** (paper × field × arm pair) from the reader with the scorer's verdict as a **feature, not a filter**, and on a constructed fixture it attempts **every** cell regardless of verdict. `effective_state` is **two-axis** (R29); `EXTRACT_FAILED` papers stay in the corpus on the **eligibility** axis with a **processing-axis reason** from S3h's closed vocabulary. The rest of the cell stands as written. |
